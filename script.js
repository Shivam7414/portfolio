/* =====================================================================
   Shivam Kumar  —  "Systems in motion"
   A small SVG diagram engine renders the hero and every project as a live
   architecture: labelled service nodes, connectors, and event packets that
   flow between them and react to hover. Plus headline reveal, scroll reveals,
   capability glyphs, and magnetic actions. No-JS / reduced-motion safe.
   ===================================================================== */
(function () {
  "use strict";
  ...
  /* =====================================================================
     DIAGRAM SPECS — each project drawn as the system it actually is.
     Coordinates live in a 1000 x H user space; node ids drive edges/flows.
     ===================================================================== */
  var SPECS = {
    hero: {
      h: 540,
      aria: "A backend system: a client and REST API fanning out to cache, queue, database, workers, AI, and payments.",
      nodes: [
        { id: "client",  label: "Client",   x: 120, y: 270, kind: "client" },
        { id: "api",     label: "REST API", x: 370, y: 270, kind: "hub" },
        { id: "redis",   label: "Redis",    x: 630, y: 120, kind: "store" },
        { id: "queue",   label: "Queue",    x: 630, y: 270, kind: "service" },
        { id: "mysql",   label: "MySQL",    x: 630, y: 420, kind: "store" },
        { id: "ai",      label: "AI / RAG", x: 880, y: 120, kind: "ai" },
        { id: "workers", label: "Workers",  x: 880, y: 270, kind: "service" },
        { id: "pay",     label: "Payments", x: 880, y: 420, kind: "pay" }
      ],
      edges: [["client","api"],["api","redis"],["api","queue"],["api","mysql"],["queue","workers"],["workers","ai"],["workers","pay"]],
      flows: [["client","api","redis"],["client","api","mysql"],["api","queue","workers","ai"],["workers","pay"]]
    },
    sendmepls: {
      h: 480,
  ...
  function buildDiagram(mount, spec) {
    ...
    spec.edges.forEach(function (e) {
      ...
      var line = svgEl("line", { x1: a.x, y1: a.y, x2: b.x, y2: b.y, class: "dgm__edge" });
    ...
    var flows = (spec.flows || []).map(function (seq, i) {
      ...
      return { pts: pts, segs: segs, total: total, glow: glow, core: core, t: (i / Math.max(1, seq.length)) + Math.random() * 0.4, sp: spec.speed || 0.0023 };
    });
    ...
    /* hover a node -> highlight its edges */
    spec.nodes.forEach(function (n) {
  ...
  /* ----------  Register + animate diagrams (only while in view)  ---------- */
  var diagrams = [];
  ...
  /* ----------  Capability glyphs  ---------- */
  var ICONS = [
    '<rect x="6" y="7" width="20" height="6" rx="1.6"/><rect x="6" y="19" width="20" height="6" rx="1.6"/><circle cx="10" cy="10" r="1"/><circle cx="10" cy="22" r="1"/>',
    '<circle cx="16" cy="16" r="3.1"/><circle cx="7" cy="8" r="2"/><circle cx="25" cy="9" r="2"/><circle cx="9" cy="25" r="2"/><circle cx="24" cy="24" r="2"/><path d="M13.3 14.1 8.6 9.7M18.7 14.3 23 10.7M13.6 18 10.2 23.1M18.6 18.2 22.3 22.3"/>',
    '<ellipse cx="16" cy="8" rx="9" ry="3"/><path d="M7 8v8c0 1.7 4 3 9 3s9-1.3 9-3V8"/><path d="M7 16v8c0 1.7 4 3 9 3s9-1.3 9-3v-8"/>',
    '<rect x="5" y="9" width="22" height="14" rx="2"/><path d="M5 13.5h22"/><path d="M9 19h5"/>',
    '<rect x="5" y="7" width="22" height="18" rx="2"/><path d="M5 12h22"/><circle cx="8.5" cy="9.5" r="0.9"/><circle cx="11.2" cy="9.5" r="0.9"/><path d="M10 17h12M10 21h7"/>',
    '<path d="M10 23h11a5 5 0 0 0 .8-9.94 7 7 0 0 0-13.2-1.2A4.5 4.5 0 0 0 10 23z"/><path d="M16 14.5v5"/><path d="M13.5 16.5 16 14l2.5 2.5"/>'
  ];
  document.querySelectorAll(".ledger__icon").forEach(function (el, i) {
    if (!ICONS[i]) return;
    el.innerHTML = '<svg viewBox="0 0 32 32" fill="none" stroke="currentColor" stroke-width="1.4" stroke-linecap="round" stroke-linejoin="round">' + ICONS[i] + "</svg>";
  });
  ...
  if (hasGSAP && !reduce) {
    ...
    gsap.set([".hero__kicker", ".hero__standfirst", ".hero__actions", ".hero__diagram", ".hero__strip"], { opacity: 0, y: 24 });
    ...
    tl.to(".hero__kicker", { opacity: 1, y: 0, duration: 0.7 })
      ...
      .to([".hero__standfirst", ".hero__actions"], { opacity: 1, y: 0, duration: 0.8, stagger: 0.1 }, "-=0.5")
      .to(".hero__diagram", { opacity: 1, y: 0, duration: 0.9 }, "-=0.7")
      .to(".hero__strip", { opacity: 1, y: 0, duration: 0.7 }, "-=0.5");

    if (hasST) {
      gsap.utils.toArray("[data-reveal]").forEach(function (el) {
        gsap.set(el, { opacity: 0, y: 18 });
        var iconShapes = el.querySelectorAll(".ledger__icon svg > *");
  ...
  if (fine && !reduce) {
    document.querySelectorAll(".link-cta:not(.link-cta--ghost), .header__cta, .sign__email, .footer__top").forEach(function (el) {
